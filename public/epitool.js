// EpiTool button injection (standalone plugin)
(function() {
    'use strict';
    ...
    function backendLog(message) {
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('epitool', 'Logger.log', { message: String(message) });
            }
        } catch (err) {
            if (typeof console !== 'undefined' && console.warn) {
                console.warn('[EpiTool] backendLog failed', err);
            }
        }
    }
    
    backendLog('EpiTool script loaded');
    // anti-spam state
    ...
    function applyTranslationBundle(bundle) {
        ...
        const stored = window.__EpiToolI18n || {};
        if (bundle.language) {
        ...
        window.__EpiToolI18n = stored;
    }

    function ensureEpiToolStyles() {
        if (document.getElementById('epitool-styles')) return;
        try {
            const style = document.createElement('style');
            style.id = 'epitool-styles';
            style.textContent = `
                .epitool-btn {
                    padding: 12px 24px;
                ...
                .epitool-btn:hover:not([data-disabled="1"]) {
                ...
                .epitool-btn.primary {
                    ...
                    color: #ffffff;
                    font-weight: 700;
                    box-shadow: 0 4px 15px rgba(147,51,234,0.4), inset 0 1px 0 rgba(255,255,255,0.3);
                    text-shadow: 0 1px 2px rgba(0,0,0,0.2);
                }
                .epitool-btn.primary:hover:not([data-disabled="1"]) {
                    background: linear-gradient(135deg, #d8b4fe 0%, #9333ea 100%);
                    transform: translateY(-3px) scale(1.03);
                    box-shadow: 0 8px 25px rgba(147,51,234,0.6), inset 0 1px 0 rgba(255,255,255,0.4);
                }
                @keyframes fadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
        ...
        } catch(err) { backendLog('EpiTool: Styles injection failed: ' + err); }
    }

    function ensureFontAwesome() {
        if (document.getElementById('epitool-fontawesome')) return;
        try {
            const link = document.createElement('link');
            link.id = 'epitool-fontawesome';
            link.rel = 'stylesheet';
        ...
        } catch(err) { backendLog('EpiTool: Font Awesome injection failed: ' + err); }
    }

    function showSettingsPopup() {
        if (document.querySelector('.epitool-settings-overlay') || settingsMenuPending) return;
        ...
        ensureTranslationsLoaded(false).catch(function(){ return null; }).finally(function(){
            ...
            if (document.querySelector('.epitool-settings-overlay')) return;

            try { const d = document.querySelector('.epitool-overlay'); if (d) d.remove(); } catch(_) {}
            ensureEpiToolStyles();
            ensureFontAwesome();

            const overlay = document.createElement('div');
            overlay.className = 'epitool-settings-overlay';
            ...
            modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:420px;max-width:600px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(147,51,234,0.3);animation:slideUp 0.1s ease-out;';

            const header = document.createElement('div');
            header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid rgba(147,51,234,0.3);';

            const title = document.createElement('div');
            title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
            title.textContent = t('menu.title', 'EpiTool · Menu');
            ...
            function createIconButton(id, iconClass, titleKey, titleFallback) {
                ...
                btn.id = id;
                btn.href = '#';
                btn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(147,51,234,0.1);border:1px solid rgba(147,51,234,0.3);border-radius:10px;color:#9333ea;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
                btn.innerHTML = '<i class="fa-solid ' + iconClass + '"></i>';
                btn.title = t(titleKey, titleFallback);
                btn.onmouseover = function() { this.style.background = 'rgba(147,51,234,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(147,51,234,0.3)'; this.style.borderColor = '#9333ea'; };
                btn.onmouseout = function() { this.style.background = 'rgba(147,51,234,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(147,51,234,0.3)'; };
            ...
            function createSectionLabel(key, fallback, marginTop) {
                ...
                label.style.cssText = 'font-size:12px;color:#9333ea;margin-top:' + topValue + 'px;margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:1.2px;text-align:center;';
            ...
            function createMenuButton(id, key, fallback, iconClass, isPrimary) {
                ...
                btn.id = id;
                btn.href = '#';
                btn.style.cssText = 'display:flex;align-items:center;justify-content:center;gap:8px;padding:14px 24px;background:linear-gradient(135deg, rgba(147,51,234,0.15) 0%, rgba(147,51,234,0.05) 100%);border:1px solid rgba(147,51,234,0.3);border-radius:12px;color:#fff;font-size:15px;font-weight:500;text-decoration:none;transition:all 0.3s ease;cursor:pointer;position:relative;overflow:hidden;text-align:center;';
                ...
                btn.onmouseover = function() { this.style.background = 'linear-gradient(135deg, rgba(147,51,234,0.3) 0%, rgba(147,51,234,0.15) 100%)'; this.style.transform = 'translateY(-2px)'; this.style.boxShadow = '0 8px 20px rgba(147,51,234,0.25)'; this.style.borderColor = '#9333ea'; };
                btn.onmouseout = function() { this.style.background = 'linear-gradient(135deg, rgba(147,51,234,0.15) 0%, rgba(147,51,234,0.05) 100%)'; this.style.transform = 'translateY(0)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(147,51,234,0.3)'; };
            ...
            const removeBtn = createMenuButton('lt-settings-remove-lua', 'menu.removeEpiTool', 'Remove via EpiTool', 'fa-trash-can');
            ...
            if (checkBtn) {
                checkBtn.addEventListener('click', function(e){
                    ...
                    try {
                        Millennium.callServerMethod('epitool', 'CheckForUpdatesNow', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                const msg = (payload && payload.message) ? String(payload.message) : lt('No updates available.');
                                ShowEpiToolAlert('EpiTool', msg);
                            } catch(_) {}
                        });
                    } catch(_) {}
                });
            ...
            if (discordBtn) {
                discordBtn.addEventListener('click', function(e){
                    e.preventDefault();
                    try { overlay.remove(); } catch(_) {}
                    const url = 'https://dsc.gg/epitool';
                    try {
                        Millennium.callServerMethod('epitool', 'OpenExternalUrl', { url, contentScriptQuery: '' });
                    } catch(_) {}
                });
            }
            ...
            if (fetchApisBtn) {
                fetchApisBtn.addEventListener('click', function(e){
                    ...
                    try { overlay.remove(); } catch(_) {}
                    try {
                        Millennium.callServerMethod('epitool', 'FetchFreeApisNow', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
            ...
            if (fixesMenuBtn) {
                fixesMenuBtn.addEventListener('click', function(e){
                    ...
                    try {
                        ...
                        const appid = match ? parseInt(match[1], 10) : (window.__EpiToolCurrentAppId || NaN);
                        if (isNaN(appid)) {
                            try { overlay.remove(); } catch(_) {}
                            const errText = t('menu.error.noAppId', 'Could not determine game AppID');
                            ShowEpiToolAlert('EpiTool', errText);
                            return;
                        }

                        Millennium.callServerMethod('epitool', 'GetGameInstallPath', { appid, contentScriptQuery: '' }).then(function(pathRes){
                            try {
                                ...
                                if (pathPayload && pathPayload.success && pathPayload.installPath) {
                                    ...
                                    window.__EpiToolGameInstallPath = pathPayload.installPath;
                                }
                                window.__EpiToolGameIsInstalled = isGameInstalled;
                                try { overlay.remove(); } catch(_) {}
                                showFixesLoadingPopupAndCheck(appid);
                            } catch(err) {
                                backendLog('EpiTool: GetGameInstallPath error: ' + err);
                                try { overlay.remove(); } catch(_) {}
                            }
                        }).catch(function() {
                            try { overlay.remove(); } catch(_) {}
                            const errorText = t('menu.error.getPath', 'Error getting game path');
                            ShowEpiToolAlert('EpiTool', errorText);
                        });
                    } catch(err) {
                        backendLog('EpiTool: Fixes Menu button error: ' + err);
                    }
                });
            }
            ...
            try {
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('epitool', 'HasEpiToolForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            ...
                            if (exists) {
                                const doDelete = function() {
                                    try {
                                        Millennium.callServerMethod('epitool', 'DeleteEpiToolForApp', { appid, contentScriptQuery: '' }).then(function(){
                                            try {
                                                window.__EpiToolButtonInserted = false;
                                                window.__EpiToolPresenceCheckInFlight = false;
                                                window.__EpiToolPresenceCheckAppId = undefined;
                                                addEpiToolButton();
                                                const successText = t('menu.remove.success', 'EpiTool removed for this app.');
                                                ShowEpiToolAlert('EpiTool', successText);
                                            } catch(err) {
                                                backendLog('EpiTool: post-delete cleanup failed: ' + err);
                                            }
                                        }).catch(function(err){
                                            const failureText = t('menu.remove.failure', 'Failed to remove EpiTool.');
                                            const errMsg = (err && err.message) ? err.message : failureText;
                                            ShowEpiToolAlert('EpiTool', errMsg);
                                        });
                                    } catch(err) {
                                        backendLog('EpiTool: doDelete failed: ' + err);
                                    }
                                };
                                ...
                                removeBtn.onclick = function(e){
                                    e.preventDefault();
                                    try { overlay.remove(); } catch(_) {}
                                    const confirmMessage = t('menu.remove.confirm', 'Remove via EpiTool for this game?');
                                    showEpiToolConfirm('EpiTool', confirmMessage, function(){
                                        doDelete();
                                    }, function(){
    ...
    function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
        try {
            if (!forceRefresh && window.__EpiToolI18n && window.__EpiToolI18n.ready) {
                return Promise.resolve(window.__EpiToolI18n);
            }
            if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
                window.__EpiToolI18n = window.__EpiToolI18n || { language: 'en', locales: [], strings: {}, ready: false };
                return Promise.resolve(window.__EpiToolI18n);
            }
            const targetLanguage = (typeof preferredLanguage === 'string' && preferredLanguage) ? preferredLanguage :
                ((window.__EpiToolI18n && window.__EpiToolI18n.language) || '');
            return Millennium.callServerMethod('epitool', 'GetTranslations', { language: targetLanguage, contentScriptQuery: '' }).then(function(res){
                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                ...
                return window.__EpiToolI18n;
            }).catch(function(err){
                backendLog('EpiTool: translation load failed: ' + err);
                window.__EpiToolI18n = window.__EpiToolI18n || { language: 'en', locales: [], strings: {}, ready: false };
                return window.__EpiToolI18n;
            });
        } catch(err) {
            backendLog('EpiTool: ensureTranslationsLoaded error: ' + err);
            window.__EpiToolI18n = window.__EpiToolI18n || { language: 'en', locales: [], strings: {}, ready: false };
            return Promise.resolve(window.__EpiToolI18n);
        }
    }
    ...
    function translateText(key, fallback) {
        ...
        try {
            const store = window.__EpiToolI18n;
    ...
    function showTestPopup() {
        ...
        // Avoid duplicates
        if (document.querySelector('.epitool-overlay')) return;
        // Close settings popup if open so modals don't overlap
        try { const s = document.querySelector('.epitool-settings-overlay'); if (s) s.remove(); } catch(_) {}
        
        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:400px;max-width:560px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(147,51,234,0.3);animation:slideUp 0.1s ease-out;';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:22px;color:#fff;margin-bottom:16px;font-weight:700;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.className = 'epitool-title';
        title.textContent = 'EpiTool';

        const body = document.createElement('div');
        body.style.cssText = 'font-size:14px;line-height:1.4;margin-bottom:12px;';
        body.className = 'epitool-status';
        body.textContent = lt('Working…');

        const progressWrap = document.createElement('div');
        progressWrap.style.cssText = 'background:rgba(26,26,26,0.5);height:12px;border-radius:4px;overflow:hidden;position:relative;display:none;border:1px solid rgba(147,51,234,0.3);';
        progressWrap.className = 'epitool-progress-wrap';
        const progressBar = document.createElement('div');
        progressBar.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg, #9333ea 0%, #c084fc 100%);transition:width 0.1s linear;box-shadow:0 0 10px rgba(147,51,234,0.5);';
        progressBar.className = 'epitool-progress-bar';
        progressWrap.appendChild(progressBar);

        const percent = document.createElement('div');
        percent.style.cssText = 'text-align:right;color:#8f98a0;margin-top:8px;font-size:12px;display:none;';
        percent.className = 'epitool-percent';
        percent.textContent = '0%';

        const btnRow = document.createElement('div');
        btnRow.style.cssText = 'margin-top:16px;display:flex;gap:8px;justify-content:flex-end;';
        const cancelBtn = document.createElement('a');
        cancelBtn.className = 'btnv6_blue_hoverfade btn_medium epitool-cancel-btn';
        ...
        hideBtn.className = 'btnv6_blue_hoverfade btn_medium epitool-hide-btn';
        ...
        function cancelOperation(){
            ...
            try {
                ...
                const appid = match ? parseInt(match[1], 10) : (window.__EpiToolCurrentAppId || NaN);
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('epitool', 'CancelAddViaEpiTool', { appid, contentScriptQuery: '' });
                }
            } catch(_) {}
            // Update UI to show cancelled
            const status = overlay.querySelector('.epitool-status');
            if (status) status.textContent = lt('Cancelled');
            const cancelBtn = overlay.querySelector('.epitool-cancel-btn');
            if (cancelBtn) cancelBtn.style.display = 'none';
            const hideBtn = overlay.querySelector('.epitool-hide-btn');
            if (hideBtn) hideBtn.innerHTML = `<span>${lt('Close')}</span>`;
            // Hide progress UI
            const wrap = overlay.querySelector('.epitool-progress-wrap');
            const percent = overlay.querySelector('.epitool-percent');
    ...
    function showFixesResultsPopup(data, isGameInstalled) {
        if (document.querySelector('.epitool-fixes-results-overlay')) return;
        // Close other popups
        try { const d = document.querySelector('.epitool-overlay'); if (d) d.remove(); } catch(_) {}
        try { const s = document.querySelector('.epitool-settings-overlay'); if (s) s.remove(); } catch(_) {}
        try { const f = document.querySelector('.epitool-fixes-results-overlay'); if (f) f.remove(); } catch(_) {}
        try { const l = document.querySelector('.epitool-loading-fixes-overlay'); if (l) l.remove(); } catch(_) {}

        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-fixes-results-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';

        const modal = document.createElement('div');
        modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:580px;max-width:700px;max-height:80vh;display:flex;flex-direction:column;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(147,51,234,0.3);animation:slideUp 0.1s ease-out;';

        const header = document.createElement('div');
        header.style.cssText = 'flex:0 0 auto;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid rgba(147,51,234,0.3);';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.textContent = lt('EpiTool · Fixes Menu');
        ...
        const body = document.createElement('div');
        body.style.cssText = 'flex:1 1 auto;overflow-y:auto;padding:20px;border:1px solid rgba(147,51,234,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        ...
        function createFixButton(label, text, icon, isSuccess, onClick) {
            ...
            sectionLabel.style.cssText = 'font-size:12px;color:#9333ea;margin-bottom:8px;font-weight:600;text-transform:uppercase;letter-spacing:1px;';
            sectionLabel.textContent = label;

            const btn = document.createElement('a');
            btn.href = '#';
            btn.style.cssText = 'display:flex;align-items:center;justify-content:center;gap:10px;width:100%;box-sizing:border-box;padding:14px 24px;background:linear-gradient(135deg, rgba(147,51,234,0.15) 0%, rgba(147,51,234,0.05) 100%);border:1px solid rgba(147,51,234,0.3);border-radius:12px;color:#fff;font-size:15px;font-weight:500;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        ...
        const genericSection = createFixButton(
            ...
            function(e) {
                ...
                if (genericStatus === 200 && isGameInstalled) {
                    const genericUrl = 'https://files.epitool.work/GameBypasses/' + data.appid + '.zip';
        ...
        const onlineSection = createFixButton(
            ...
            function(e) {
                ...
                if (onlineStatus === 200 && isGameInstalled) {
                    const onlineUrl = data.onlineFix.url || ('https://files.epitool.work/OnlineFix1/' + data.appid + '.zip');
        ...
        const unfixSection = createFixButton(
            ...
            function(e) {
                ...
                if (isGameInstalled) {
                    try { overlay.remove(); } catch(_) {}
                    showEpiToolConfirm('EpiTool', lt('Are you sure you want to un-fix? This will remove fix files and verify game files.'),
        ...
        gameFolderBtn.onclick = function(e){ 
            e.preventDefault(); 
            if (window.__EpiToolGameInstallPath) {
                try {
                    Millennium.callServerMethod('epitool', 'OpenGameFolder', { path: window.__EpiToolGameInstallPath, contentScriptQuery: '' });
                } catch(err) { backendLog('EpiTool: Failed to open game folder: ' + err); }
            }
        };
        ...
        discordBtn.onclick = function(e) {
            ...
            try { overlay.remove(); } catch(_) {}
            const url = 'https://dsc.gg/epitool';
            try { Millennium.callServerMethod('epitool', 'OpenExternalUrl', { url, contentScriptQuery: '' }); } catch(_) {}
        };
        ...
        function startUnfix(appid) {
            try {
                Millennium.callServerMethod('epitool', 'UnFixGame', { appid: appid, installPath: window.__EpiToolGameInstallPath, contentScriptQuery: '' }).then(function(res){
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        ...
                        ShowEpiToolAlert('EpiTool', errorMsg);
                    }
                }).catch(function(){
                    const msg = lt('Error starting un-fix');
                    ShowEpiToolAlert('EpiTool', msg);
                });
            } catch(err) { backendLog('EpiTool: Un-Fix start error: ' + err); }
        }
    }

    function showFixesLoadingPopupAndCheck(appid) {
        if (document.querySelector('.epitool-loading-fixes-overlay')) return;
        try { const d = document.querySelector('.epitool-overlay'); if (d) d.remove(); } catch(_) {}
        try { const s = document.querySelector('.epitool-settings-overlay'); if (s) s.remove(); } catch(_) {}
        try { const f = document.querySelector('.epitool-fixes-overlay'); if (f) f.remove(); } catch(_) {}

        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-loading-fixes-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        const progressWrap = document.createElement('div');
        progressWrap.style.cssText = 'background:rgba(26,26,26,0.5);height:12px;border-radius:4px;overflow:hidden;position:relative;border:1px solid rgba(147,51,234,0.3);';
        const progressBar = document.createElement('div');
        progressBar.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg, #9333ea 0%, #c084fc 100%);transition:width 0.2s linear;box-shadow:0 0 10px rgba(147,51,234,0.5);';
        progressWrap.appendChild(progressBar);
        ...
        Millennium.callServerMethod('epitool', 'CheckForFixes', { appid, contentScriptQuery: '' }).then(function(res){
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (payload && payload.success) {
                const isGameInstalled = window.__EpiToolGameIsInstalled === true;
            ...
            setTimeout(function() {
                try {
                    const l = document.querySelector('.epitool-loading-fixes-overlay');
                    if (l) l.remove();
                } catch(_) {}
            }, 300);
        });
    ...
    function applyFix(appid, downloadUrl, fixType, gameName, resultsOverlay) {
        try {
            ...
            backendLog('EpiTool: Applying fix ' + fixType + ' for appid ' + appid);
            
            // Start the download and extraction process
            Millennium.callServerMethod('epitool', 'ApplyGameFix', { 
                appid: appid, 
                downloadUrl: downloadUrl, 
                installPath: window.__EpiToolGameInstallPath,
                fixType: fixType,
                gameName: gameName || '',
                contentScriptQuery: '' 
            }).then(function(res){
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        // Show download progress popup similar to Add via EpiTool
                    ...
                    backendLog('EpiTool: ApplyGameFix response error: ' + err);
                    const msg = lt('Error applying fix');
                    ShowEpiToolAlert('EpiTool', msg);
                }
            }).catch(function(err){
                backendLog('EpiTool: ApplyGameFix error: ' + err);
                const msg = lt('Error applying fix');
                ShowEpiToolAlert('EpiTool', msg);
            });
        } catch(err) {
            backendLog('EpiTool: applyFix error: ' + err);
        }
    }

    // Show fix download progress popup
    function showFixDownloadProgress(appid, fixType) {
        // Reuse the download popup UI from Add via EpiTool
        if (document.querySelector('.epitool-overlay')) return;
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'epitool-btn primary';
        ...
        cancelBtn.onclick = function(e){
            ...
            Millennium.callServerMethod('epitool', 'CancelApplyFix', { appid: appid, contentScriptQuery: '' }).then(function(res){
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    ...
                    backendLog('EpiTool: CancelApplyFix response error: ' + err);
                ...
                const msgEl2 = document.getElementById('lt-fix-progress-msg');
                if (msgEl2 && msgEl2.dataset.last) msgEl2.textContent = msgEl2.dataset.last;
                backendLog('EpiTool: CancelApplyFix error: ' + err);
                const msg = lt('Failed to cancel fix download');
                ShowEpiToolAlert('EpiTool', msg);
            });
        };
    ...
    function replaceFixButtonsWithClose(overlayEl) {
        ...
        closeBtn.href = '#';
        closeBtn.className = 'epitool-btn primary';
    ...
    function pollFixProgress(appid, fixType) {
        const poll = function() {
            try {
                const overlayEl = document.querySelector('.epitool-overlay');
                if (!overlayEl) return; // Stop if overlay was closed
                
                Millennium.callServerMethod('epitool', 'GetApplyFixStatus', { appid: appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        ...
                        backendLog('EpiTool: GetApplyFixStatus error: ' + err);
                    }
                });
            } catch(err) {
                backendLog('EpiTool: pollFixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function showUnfixProgress(appid) {
        ...
        try { const old = document.querySelector('.epitool-unfix-overlay'); if (old) old.remove(); } catch(_) {}

        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-unfix-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
    ...
    function pollUnfixProgress(appid) {
        const poll = function() {
            try {
                const overlayEl = document.querySelector('.epitool-unfix-overlay');
                if (!overlayEl) return; // Stop if overlay was closed
                
                Millennium.callServerMethod('epitool', 'GetUnfixStatus', { appid: appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.success && payload.state) {
                            const state = payload.state;
                            ...
                            if (state.status === 'removing') {
                                ...
                                setTimeout(function(){
                                    try {
                                        ...
                                        backendLog('EpiTool: Running verify for appid ' + appid);
                                    } catch(_) {}
                                }, 1000);
                        ...
                        backendLog('EpiTool: GetUnfixStatus error: ' + err);
                    }
                });
            } catch(err) {
                backendLog('EpiTool: pollUnfixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function fetchSettingsConfig(forceRefresh) {
        try {
            if (!forceRefresh && window.__EpiToolSettings && Array.isArray(window.__EpiToolSettings.schema)) {
                return Promise.resolve(window.__EpiToolSettings);
            }
        } catch(_) {}

        if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
            return Promise.reject(new Error(lt('EpiTool backend unavailable')));
        }

        return Millennium.callServerMethod('epitool', 'GetSettingsConfig', { contentScriptQuery: '' }).then(function(res){
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (!payload || payload.success !== true) {
            ...
            window.__EpiToolSettings = config;
            return config;
        });
    }
    ...
    function showSettingsManagerPopup(forceRefresh, onBack) {
        if (document.querySelector('.epitool-settings-manager-overlay')) return;

        try { const mainOverlay = document.querySelector('.epitool-settings-overlay'); if (mainOverlay) mainOverlay.remove(); } catch(_) {}

        ensureEpiToolStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'epitool-settings-manager-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:650px;max-width:750px;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(147,51,234,0.3);animation:slideUp 0.1s ease-out;overflow:hidden;';

        const header = document.createElement('div');
        header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding:28px 32px 16px;border-bottom:2px solid rgba(147,51,234,0.3);';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.textContent = t('settings.title', 'EpiTool · Settings');
        ...
        discordIconBtn.href = '#';
        discordIconBtn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(147,51,234,0.1);border:1px solid rgba(147,51,234,0.3);border-radius:10px;color:#9333ea;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        discordIconBtn.innerHTML = '<i class="fa-brands fa-discord"></i>';
        discordIconBtn.title = t('menu.discord', 'Discord');
        discordIconBtn.onmouseover = function() { this.style.background = 'rgba(147,51,234,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(147,51,234,0.3)'; this.style.borderColor = '#9333ea'; };
        discordIconBtn.onmouseout = function() { this.style.background = 'rgba(147,51,234,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(147,51,234,0.3)'; };
        iconButtons.appendChild(discordIconBtn);

        const closeIconBtn = document.createElement('a');
        closeIconBtn.href = '#';
        closeIconBtn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(147,51,234,0.1);border:1px solid rgba(147,51,234,0.3);border-radius:10px;color:#9333ea;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        closeIconBtn.innerHTML = '<i class="fa-solid fa-xmark"></i>';
        closeIconBtn.title = t('settings.close', 'Close');
        closeIconBtn.onmouseover = function() { this.style.background = 'rgba(147,51,234,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(147,51,234,0.3)'; this.style.borderColor = '#9333ea'; };
        closeIconBtn.onmouseout = function() { this.style.background = 'rgba(147,51,234,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(147,51,234,0.3)'; };
        iconButtons.appendChild(closeIconBtn);

        const contentWrap = document.createElement('div');
        contentWrap.style.cssText = 'flex:1 1 auto;overflow-y:auto;overflow-x:hidden;padding:20px;margin:0 24px;border:1px solid rgba(147,51,234,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        ...
        function setStatus(text, color) {
            let statusLine = contentWrap.querySelector('.epitool-settings-status');
            if (!statusLine) {
                statusLine = document.createElement('div');
                statusLine.className = 'epitool-settings-status';
        ...
        function renderSettings() {
            ...
            if (!state.config || !Array.isArray(state.config.schema) || state.config.schema.length === 0) {
                ...
                emptyState.style.cssText = 'padding:14px;background:#0a0a0a;border:1px solid #1a1a1a;border-radius:4px;color:#c7d5e0;';
            ...
            for (let i = 0; i < state.config.schema.length; i++) {
                ...
                if (group.key === 'general') {
                    ...
                    groupTitle.style.cssText = 'font-size:15px;font-weight:600;color:#9333ea;text-align:center;';
                }
                ...
                for (let j = 0; j < options.length; j++) {
                    ...
                    if (j === 0) {
                        ...
                        optionEl.style.cssText = 'margin-top:12px;padding-top:12px;border-top:1px solid rgba(147,51,234,0.1);';
                    }
                    ...
                    if (option.type === 'select') {
                        ...
                        selectEl.style.cssText = 'width:100%;padding:6px 8px;background:#0f0f0f;color:#dfe6f0;border:1px solid #1a1a1a;border-radius:3px;';
                        ...
                        selectEl.addEventListener('change', function(){
                            ...
                            try { backendLog('EpiTool: language select changed to ' + selectEl.value); } catch(_) {}
                            updateSaveState();
                        ...
                        function refreshToggleButtons() {
                            ...
                            if (currentValue) {
                                yesBtn.style.background = '#9333ea';
        ...
        function renderInstalledFixesSection() {
            ...
            sectionEl.id = 'epitool-installed-fixes-section';
            sectionEl.style.cssText = 'margin-top:36px;padding:24px;background:linear-gradient(135deg, rgba(147,51,234,0.05) 0%, rgba(74,158,206,0.08) 100%);border:2px solid rgba(74,158,206,0.3);border-radius:14px;box-shadow:0 4px 15px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05);position:relative;overflow:hidden;';

            const sectionGlow = document.createElement('div');
            sectionGlow.style.cssText = 'position:absolute;top:-100%;left:-100%;width:300%;height:300%;background:radial-gradient(circle, rgba(147,51,234,0.08) 0%, transparent 70%);pointer-events:none;';
            sectionEl.appendChild(sectionGlow);

            const sectionTitle = document.createElement('div');
            sectionTitle.style.cssText = 'font-size:22px;color:#9333ea;margin-bottom:20px;font-weight:700;text-align:center;text-shadow:0 2px 10px rgba(147,51,234,0.5);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;position:relative;z-index:1;letter-spacing:0.5px;';
            ...
            listContainer.id = 'epitool-fixes-list';
        ...
        function loadInstalledFixes(container) {
            ...
            Millennium.callServerMethod('epitool', 'GetInstalledFixes', { contentScriptQuery: '' })
                .then(function(res) {
                    const response = typeof res === 'string' ? JSON.parse(res) : res;
                    if (!response || !response.success) {
                        container.innerHTML = '<div style="padding:14px;background:#0a0a0a;border:1px solid #ff5c5c;border-radius:4px;color:#ff5c5c;">' + t('settings.installedFixes.error', 'Failed to load installed fixes.') + '</div>';
                        return;
                    }

                    const fixes = Array.isArray(response.fixes) ? response.fixes : [];
                    if (fixes.length === 0) {
                        container.innerHTML = '<div style="padding:14px;background:#0a0a0a;border:1px solid #1a1a1a;border-radius:4px;color:#c7d5e0;text-align:center;">' + t('settings.installedFixes.empty', 'No fixes installed yet.') + '</div>';
                        return;
                    }
        ...
        function createFixListItem(fix, container) {
            ...
            itemEl.style.cssText = 'margin-bottom:12px;padding:14px;background:rgba(11,20,30,0.8);border:1px solid rgba(147,51,234,0.3);border-radius:6px;display:flex;justify-content:space-between;align-items:center;transition:all 0.2s ease;';
            itemEl.onmouseover = function() { this.style.borderColor = '#9333ea'; this.style.background = 'rgba(11,20,30,0.95)'; };
            itemEl.onmouseout = function() { this.style.borderColor = 'rgba(147,51,234,0.3)'; this.style.background = 'rgba(11,20,30,0.8)'; };
            ...
            if (fix.fixType) {
                ...
                typeSpan.innerHTML = '<strong style="color:#9333ea;">' + t('settings.installedFixes.type', 'Type:') + '</strong> ' + fix.fixType;
                detailsDiv.appendChild(typeSpan);
            }

            if (fix.date) {
                const dateSpan = document.createElement('div');
                dateSpan.innerHTML = '<strong style="color:#9333ea;">' + t('settings.installedFixes.date', 'Installed:') + '</strong> ' + fix.date;
                detailsDiv.appendChild(dateSpan);
            }

            if (fix.filesCount > 0) {
                const filesSpan = document.createElement('div');
                filesSpan.innerHTML = '<strong style="color:#9333ea;">' + t('settings.installedFixes.files', '{count} files').replace('{count}', fix.filesCount) + '</strong>';
            ...
            deleteBtn.addEventListener('click', function(e) {
                ...
                showEpiToolConfirm(
                    ...
                    function() {
                        ...
                        Millennium.callServerMethod('epitool', 'UnFixGame', {
                            appid: fix.appid,
        ...
        function pollUnfixStatus(appid, itemEl, deleteBtn, container) {
            ...
            function checkStatus() {
                ...
                Millennium.callServerMethod('epitool', 'GetUnfixStatus', { appid: appid, contentScriptQuery: '' })
                    .then(function(res) {
                        const response = typeof res === 'string' ? JSON.parse(res) : res;
                        if (!response || !response.success) {
        ...
        function renderInstalledLuaSection() {
            const sectionEl = document.createElement('div');
            sectionEl.id = 'epitool-installed-lua-section';
            ...
            const listContainer = document.createElement('div');
            listContainer.id = 'epitool-lua-list';
            listContainer.style.cssText = 'min-height:50px;';
            sectionEl.appendChild(listContainer);
        ...
        function loadInstalledLuaScripts(container) {
            ...
            Millennium.callServerMethod('epitool', 'GetInstalledLuaScripts', { contentScriptQuery: '' })
                .then(function(res) {
                    const response = typeof res === 'string' ? JSON.parse(res) : res;
                    if (!response || !response.success) {
                        container.innerHTML = '<div style="padding:14px;background:#0a0a0a;border:1px solid #ff5c5c;border-radius:4px;color:#ff5c5c;">' + t('settings.installedLua.error', 'Failed to load installed Lua scripts.') + '</div>';
                        return;
                    }

                    const scripts = Array.isArray(response.scripts) ? response.scripts : [];
                    if (scripts.length === 0) {
                        container.innerHTML = '<div style="padding:14px;background:#0a0a0a;border:1px solid #1a1a1a;border-radius:4px;color:#c7d5e0;text-align:center;">' + t('settings.installedLua.empty', 'No Lua scripts installed yet.') + '</div>';
                        return;
                    }
                    ...
                    if (hasUnknownGames) {
                        ...
                        infoBanner.innerHTML = '<i class="fa-solid fa-circle-info" style="font-size:16px;"></i><span>' + t('settings.installedLua.unknownInfo', 'Games showing \'Unknown Game\' were installed manually (not via EpiTool).') + '</span>';
        ...
        function createLuaListItem(script, container) {
            ...
            if (script.modifiedDate) {
                const dateSpan = document.createElement('div');
                dateSpan.innerHTML = '<strong style="color:#9333ea;">' + t('settings.installedLua.modified', 'Modified:') + '</strong> ' + script.modifiedDate;
                detailsDiv.appendChild(dateSpan);
            }
            ...
            deleteBtn.addEventListener('click', function(e) {
                ...
                showEpiToolConfirm(
                    script.gameName || 'EpiTool',
                    t('settings.installedLua.deleteConfirm', 'Remove via EpiTool for this game?'),
                    function() {
                        // User confirmed
                        deleteBtn.dataset.busy = '1';
                        deleteBtn.style.opacity = '0.6';
                        ...
                        Millennium.callServerMethod('epitool', 'DeleteEpiToolForApp', {
                            appid: script.appid,
                            contentScriptQuery: ''
                        })
                        .then(function(res) {
        ...
        saveBtn.addEventListener('click', function(e){
            ...
            try { backendLog('EpiTool: collectChanges payload ' + JSON.stringify(changes)); } catch(_) {}
            ...
            try { backendLog('EpiTool: sending settings payload ' + JSON.stringify(payloadToSend)); } catch(_) {}
            // Pass flattened keys so Millennium handles the RPC arguments as expected.
            Millennium.callServerMethod('epitool', 'ApplySettingsChanges', {
                contentScriptQuery: '',
                ...
                try {
                    if (window.__EpiToolSettings) {
                        window.__EpiToolSettings.values = JSON.parse(JSON.stringify(state.config.values));
                        window.__EpiToolSettings.schemaVersion = state.config.schemaVersion;
                        window.__EpiToolSettings.lastFetched = Date.now();
                        if (response && response.translations && typeof response.translations === 'object') {
                            window.__EpiToolSettings.translations = response.translations;
                        }
                        if (response && response.language) {
                            window.__EpiToolSettings.language = response.language;
                        }
                    }
                } catch(_) {}
                ...
                if (response && response.translations && typeof response.translations === 'object') {
                    applyTranslationBundle({
                        language: response.language || (window.__EpiToolI18n && window.__EpiToolI18n.language) || 'en',
                        locales: (window.__EpiToolI18n && window.__EpiToolI18n.locales) || (state.config && state.config.locales) || [],
    ...
    function closeSettingsOverlay() {
        try {
            ...
            var list = document.getElementsByClassName('epitool-settings-overlay');
            while (list && list.length > 0) {
                try { list[0].remove(); } catch(_) { break; }
            }
            // Also remove any download/progress overlays if present
            var list2 = document.getElementsByClassName('epitool-overlay');
    ...
    function showEpiToolAlert(title, message, onClose) {
        if (document.querySelector('.epitool-alert-overlay')) return;

        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-alert-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:400px;max-width:520px;padding:32px 36px;box-shadow:0 20px 60px rgba(0,0,0,.9), 0 0 0 1px rgba(147,51,234,0.4);animation:slideUp 0.1s ease-out;';

        const titleEl = document.createElement('div');
        titleEl.style.cssText = 'font-size:22px;color:#fff;margin-bottom:20px;font-weight:700;text-align:left;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        titleEl.textContent = String(title || 'EpiTool');
        ...
        okBtn.href = '#';
        okBtn.className = 'epitool-btn primary';
    ...
    function ShowEpiToolAlert(title, message) {
        try {
            showEpiToolAlert(title, message);
        } catch(err) {
            backendLog('EpiTool: Alert error, falling back: ' + err);
            try { alert(String(title) + '\n\n' + String(message)); } catch(_) {}
        }
    }

    // Steam-style confirm helper (ShowConfirmDialog only)
    function showEpiToolConfirm(title, message, onConfirm, onCancel) {
        // Always close settings popup first so the confirm is visible on top
        closeSettingsOverlay();

        // Create custom modern confirmation dialog
        if (document.querySelector('.epitool-confirm-overlay')) return;

        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.className = 'epitool-confirm-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';

        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:420px;max-width:540px;padding:32px 36px;box-shadow:0 20px 60px rgba(0,0,0,.9), 0 0 0 1px rgba(147,51,234,0.4);animation:slideUp 0.1s ease-out;';

        const titleEl = document.createElement('div');
        titleEl.style.cssText = 'font-size:22px;color:#fff;margin-bottom:20px;font-weight:700;text-align:center;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        titleEl.textContent = String(title || 'EpiTool');
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'epitool-btn';
        cancelBtn.style.flex = '1';
        cancelBtn.innerHTML = `<span>${lt('Cancel')}</span>`;
        cancelBtn.onclick = function(e) {
            e.preventDefault();
        ...
        confirmBtn.href = '#';
        confirmBtn.className = 'epitool-btn primary';
    ...
    function ensureStyles() {
        if (!document.getElementById('epitool-spacing-styles')) {
            const style = document.createElement('style');
            style.id = 'epitool-spacing-styles';
            style.textContent = '.epitool-restart-button, .epitool-button, .epitool-icon-button{ margin-left:6px !important; }';
    ...
    function updateButtonTranslations() {
        try {
            ...
            const restartBtn = document.querySelector('.epitool-restart-button');
            if (restartBtn) {
            ...
            // Update Add via EpiTool button
            const epitoolBtn = document.querySelector('.epitool-button');
            if (epitoolBtn) {
                const addViaText = lt('Add via EpiTool');
                epitoolBtn.title = addViaText;
                epitoolBtn.setAttribute('data-tooltip-text', addViaText);
                const span = epitoolBtn.querySelector('span');
                if (span) {
                    span.textContent = addViaText;
                }
            }
        } catch(err) {
            backendLog('EpiTool: updateButtonTranslations error: ' + err);
        }
    }

    // Function to add the EpiTool button
    function addEpiToolButton() {
        // Track current URL to detect page changes
        const currentUrl = window.location.href;
        if (window.__EpiToolLastUrl !== currentUrl) {
            // Page changed - reset button insertion flag and update translations
            window.__EpiToolLastUrl = currentUrl;
            window.__EpiToolButtonInserted = false;
            window.__EpiToolRestartInserted = false;
            window.__EpiToolIconInserted = false;
            window.__EpiToolPresenceCheckInFlight = false;
            window.__EpiToolPresenceCheckAppId = undefined;
        ...
        if (steamdbContainer) {
            ...
            const existingBtn = document.querySelector('.epitool-button');
            if (existingBtn) {
                ensureTranslationsLoaded(false).then(function() {
                    updateButtonTranslations();
                });
            ...
            if (existingBtn || window.__EpiToolButtonInserted) {
                if (!logState.existsOnce) { backendLog('EpiTool button already exists, skipping'); logState.existsOnce = true; }
                // Even if EpiTool exists, ensure Restart button is present and translations are updated
                return;
            }

            // Insert a Restart Steam button between Community Hub and our EpiTool button
            try {
                if (!document.querySelector('.epitool-restart-button') && !window.__EpiToolRestartInserted) {
                    ensureStyles();
                    const referenceBtn = steamdbContainer.querySelector('a');
                    const restartBtn = document.createElement('a');
                    if (referenceBtn && referenceBtn.className) {
                        restartBtn.className = referenceBtn.className + ' epitool-restart-button';
                    } else {
                        restartBtn.className = 'btnv6_blue_hoverfade btn_medium epitool-restart-button';
                    }
                    restartBtn.href = '#';
                    const restartText = lt('Restart Steam');
                    restartBtn.title = restartText;
                    ...
                    restartBtn.addEventListener('click', function(e){
                        ...
                        try {
                            ...
                            closeSettingsOverlay();
                            showEpiToolConfirm('EpiTool', lt('Restart Steam now?'),
                                function() { try { Millennium.callServerMethod('epitool', 'RestartSteam', { contentScriptQuery: '' }); } catch(_) {} },
                    ...
                    try {
                        if (!document.querySelector('.epitool-icon-button') && !window.__EpiToolIconInserted) {
                            const iconBtn = document.createElement('a');
                            if (referenceBtn && referenceBtn.className) {
                                iconBtn.className = referenceBtn.className + ' epitool-icon-button';
                            } else {
                                iconBtn.className = 'btnv6_blue_hoverfade btn_medium epitool-icon-button';
                            }
                            iconBtn.href = '#';
                            iconBtn.title = 'EpiTool Helper';
                            iconBtn.setAttribute('data-tooltip-text', 'EpiTool Helper');
                            // Normalize margins to match native buttons
                            try {
                                if (referenceBtn) {
                                    const cs = window.getComputedStyle(referenceBtn);
                            ...
                            try {
                                Millennium.callServerMethod('epitool', 'GetIconDataUrl', { contentScriptQuery: '' }).then(function(res){
                                    try {
                                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                        if (payload && payload.success && payload.dataUrl) {
                                            img.src = payload.dataUrl;
                                        } else {
                                            img.src = 'EpiTool/epitool-icon.png';
                                        }
                                    } catch(_) { img.src = 'EpiTool/epitool-icon.png'; }
                                });
                            } catch(_) {
                                img.src = 'EpiTool/epitool-icon.png';
                            }
                            ...
                            window.__EpiToolIconInserted = true;
                            backendLog('Inserted Icon button');
                        }
                    } catch(_) {}
                    window.__EpiToolRestartInserted = true;
                    backendLog('Inserted Restart Steam button');
                }
            } catch(_) {}

            // If EpiTool button already existed, stop here
            if (document.querySelector('.epitool-button') || window.__EpiToolButtonInserted) {
                return;
            }
            
            // Create the EpiTool button modeled after existing SteamDB/PCGW buttons
            let referenceBtn = steamdbContainer.querySelector('a');
            const epitoolButton = document.createElement('a');
            epitoolButton.href = '#';
            // Copy classes from an existing button to match look-and-feel, but set our own label
            if (referenceBtn && referenceBtn.className) {
                epitoolButton.className = referenceBtn.className + ' epitool-button';
            } else {
                epitoolButton.className = 'btnv6_blue_hoverfade btn_medium epitool-button';
            }
            const span = document.createElement('span');
            const addViaText = lt('Add via EpiTool');
            span.textContent = addViaText;
            epitoolButton.appendChild(span);
            // Tooltip/title
            epitoolButton.title = addViaText;
            epitoolButton.setAttribute('data-tooltip-text', addViaText);
            // Normalize margins to match native buttons
            try {
                if (referenceBtn) {
                    const cs = window.getComputedStyle(referenceBtn);
                    epitoolButton.style.marginLeft = cs.marginLeft;
                    epitoolButton.style.marginRight = cs.marginRight;
                }
            } catch(_) {}
            
            // Local click handler suppressed; delegated handler manages actions
            epitoolButton.addEventListener('click', function(e) {
                e.preventDefault();
                backendLog('EpiTool button clicked (delegated handler will process)');
            });
            
            // Before inserting, ask backend if EpiTool already exists for this appid
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                const appid = match ? parseInt(match[1], 10) : NaN;
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    // prevent multiple concurrent checks
                    if (window.__EpiToolPresenceCheckInFlight && window.__EpiToolPresenceCheckAppId === appid) {
                        return;
                    }
                    window.__EpiToolPresenceCheckInFlight = true;
                    window.__EpiToolPresenceCheckAppId = appid;
                    window.__EpiToolCurrentAppId = appid;
                    Millennium.callServerMethod('epitool', 'HasEpiToolForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            if (payload && payload.success && payload.exists === true) {
                                backendLog('EpiTool already present for this app; not inserting button');
                                window.__EpiToolPresenceCheckInFlight = false;
                                return; // do not insert
                            }
                            // Re-check in case another caller inserted during async
                            if (!document.querySelector('.epitool-button') && !window.__EpiToolButtonInserted) {
                                const restartExisting = steamdbContainer.querySelector('.epitool-restart-button');
                                if (restartExisting && restartExisting.after) {
                                    restartExisting.after(epitoolButton);
                                } else if (referenceBtn && referenceBtn.after) {
                                    referenceBtn.after(epitoolButton);
                                } else {
                                    steamdbContainer.appendChild(epitoolButton);
                                }
                                window.__EpiToolButtonInserted = true;
                                backendLog('EpiTool button inserted');
                            }
                            window.__EpiToolPresenceCheckInFlight = false;
                        } catch(_) {
                            if (!document.querySelector('.epitool-button') && !window.__EpiToolButtonInserted) {
            ...
            if (!logState.missingOnce) { backendLog('EpiTool: steamdbContainer not found on this page'); logState.missingOnce = true; }
        }
    }
    ...
    function onFrontendReady() {
        ...
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('epitool', 'GetInitApisMessage', { contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.message) {
                            ...
                            if (isUpdateMsg) {
                                ...
                                showEpiToolConfirm('EpiTool', msg, function() {
                                    // User clicked Confirm - restart Steam
                                    try { Millennium.callServerMethod('epitool', 'RestartSteam', { contentScriptQuery: '' }); } catch(_) {}
                                }, function() {
                ...
                try {
                    if (!sessionStorage.getItem('EpiToolLoadedAppsGate')) {
                        sessionStorage.setItem('EpiToolLoadedAppsGate', '1');
                        Millennium.callServerMethod('epitool', 'ReadLoadedApps', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    document.addEventListener('click', function(evt) {
        const anchor = evt.target && (evt.target.closest ? evt.target.closest('.epitool-button') : null);
        if (anchor) {
            evt.preventDefault();
            backendLog('EpiTool delegated click');
            // Use the same loading modal on delegated clicks
            if (!document.querySelector('.epitool-overlay')) {
                showTestPopup();
            }
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    if (runState.inProgress && runState.appid === appid) {
                        backendLog('EpiTool: operation already in progress for this appid');
                        return;
                    }
                    runState.inProgress = true;
                    runState.appid = appid;
                    Millennium.callServerMethod('epitool', 'StartAddViaEpiTool', { appid, contentScriptQuery: '' });
                    startPolling(appid);
                }
            } catch(_) {}
        }
    ...
    function startPolling(appid){
        ...
        const timer = setInterval(() => {
            ...
            try {
                Millennium.callServerMethod('epitool', 'GetAddViaEpiToolStatus', { appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        const st = payload && payload.state ? payload.state : {};
                        
                        // Try to find overlay (may or may not be visible)
                        const overlay = document.querySelector('.epitool-overlay');
                        const title = overlay ? overlay.querySelector('.epitool-title') : null;
                        const status = overlay ? overlay.querySelector('.epitool-status') : null;
                        const wrap = overlay ? overlay.querySelector('.epitool-progress-wrap') : null;
                        const percent = overlay ? overlay.querySelector('.epitool-percent') : null;
                        const bar = overlay ? overlay.querySelector('.epitool-progress-bar') : null;
                        
                        // Update UI if overlay is present
                        if (st.currentApi && title) title.textContent = lt('EpiTool · {api}').replace('{api}', st.currentApi);
                        if (status) {
                        ...
                        if (st.status === 'downloading'){
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.epitool-cancel-btn') : null;
                        ...
                        if (st.status === 'done'){
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.epitool-cancel-btn') : null;
                            if (cancelBtn) cancelBtn.style.display = 'none';
                            const hideBtn = overlay ? overlay.querySelector('.epitool-hide-btn') : null;
                            if (hideBtn) hideBtn.innerHTML = '<span>' + lt('Close') + '</span>';
                            ...
                            const btnEl = document.querySelector('.epitool-button');
    ...
    setTimeout(addEpiToolButton, 1000);
    setTimeout(addEpiToolButton, 3000);
    ...
    function showLoadedAppsPopup(apps) {
        // Avoid duplicates
        if (document.querySelector('.epitool-loadedapps-overlay')) return;
        ensureEpiToolStyles();
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'epitool-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'epitool-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #9333ea;border-radius:8px;min-width:420px;max-width:640px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(147,51,234,0.3);animation:slideUp 0.1s ease-out;';
        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;margin-bottom:20px;font-weight:700;text-shadow:0 2px 8px rgba(147,51,234,0.4);background:linear-gradient(135deg, #9333ea 0%, #c084fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;text-align:center;';
        title.textContent = lt('EpiTool · Added Games');
        const body = document.createElement('div');
        body.style.cssText = 'font-size:14px;line-height:1.8;margin-bottom:16px;max-height:320px;overflow:auto;padding:16px;border:1px solid rgba(147,51,234,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        if (apps && apps.length) {
            ...
            apps.forEach(function(item){
                ...
                a.style.cssText = 'display:block;color:#c7d5e0;text-decoration:none;padding:10px 16px;margin-bottom:8px;background:rgba(147,51,234,0.08);border:1px solid rgba(147,51,234,0.2);border-radius:4px;transition:all 0.3s ease;';
                a.onmouseover = function() { this.style.background = 'rgba(147,51,234,0.2)'; this.style.borderColor = '#9333ea'; this.style.transform = 'translateX(4px)'; this.style.color = '#fff'; };
                a.onmouseout = function() { this.style.background = 'rgba(147,51,234,0.08)'; this.style.borderColor = 'rgba(147,51,234,0.2)'; this.style.transform = 'translateX(0)'; this.style.color = '#c7d5e0'; };
        ...
        dismissBtn.href = '#';
        dismissBtn.onclick = function(e){ e.preventDefault(); try { Millennium.callServerMethod('epitool', 'DismissLoadedApps', { contentScriptQuery: '' }); } catch(_) {} try { sessionStorage.setItem('EpiToolLoadedAppsShown', '1'); } catch(_) {} overlay.remove(); };
