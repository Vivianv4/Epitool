#!/bin/bash
# Build Script for EpiTool Steam Plugin (Linux/Mac)
...
# Configuration
OUTPUT_NAME="${1:-epitoolplugin.zip}"
CLEAN="${2:-false}"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_PATH="${ROOT_DIR}/${OUTPUT_NAME}"

echo -e "${CYAN}=== EpiTool Build Script ===${NC}"
...
if command -v python3 &> /dev/null && [ -f "${ROOT_DIR}/scripts/validate_locales.py" ]; then
    cd "$ROOT_DIR"
    ...
    echo -e "${YELLOW}WARNING: Python not found or validation script missing, skipping locale validation${NC}"
fi
