"""Management of the EpiTool API manifest (free API list)."""
...
def fetch_free_apis_now(content_script_query: str = "") -> str:
    ...
    client = ensure_http_client("EpiTool: FetchFreeApisNow")
    try:
        logger.log("EpiTool: FetchFreeApisNow invoked")
        manifest_text = ""
        ...
        try:
            ...
            resp.raise_for_status()
            manifest_text = resp.text
            logger.log("EpiTool: Fetched manifest from primary URL")
        except Exception as primary_err:
            logger.warn(f"EpiTool: Primary manifest URL failed ({primary_err}), trying proxy...")
            try:
                resp = client.get(
                    API_MANIFEST_PROXY_URL,
                ...
                resp.raise_for_status()
                manifest_text = resp.text
                logger.log("EpiTool: Fetched manifest from proxy URL")
            except Exception as proxy_err:
                logger.warn(f"EpiTool: Proxy manifest URL also failed: {proxy_err}")
                return json.dumps(
        ...
        logger.warn(f"EpiTool: FetchFreeApisNow failed: {exc}")
...
def load_api_manifest() -> List[Dict[str, Any]]:
    ...
    if normalized and normalized != text:
        try:
            ...
            logger.log("EpiTool: Normalized api.json to valid JSON")
        except Exception:
            pass
        text = normalized
    ...
    try:
        ...
        logger.error(f"EpiTool: Failed to parse api.json: {exc}")
        return []
