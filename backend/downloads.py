"""Handling of EpiTool add/download flows and related utilities."""
...
def _fetch_app_name(appid: int) -> str:
    ...
    client = ensure_http_client("EpiTool: _fetch_app_name")
    try:
        ...
        logger.warn(f"EpiTool: _fetch_app_name failed for {appid}: {exc}")
...
def _append_loaded_app(appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"EpiTool: _append_loaded_app failed for {appid}: {exc}")
...
def _remove_loaded_app(appid: int) -> None:
    try:
        ...
        logger.warn(f"EpiTool: _remove_loaded_app failed for {appid}: {exc}")
...
def _log_appid_event(action: str, appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"EpiTool: _log_appid_event failed: {exc}")
...
def _preload_app_names_cache() -> None:
    ...
    try:
        ...
        logger.warn(f"EpiTool: _preload_app_names_cache from logs failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"EpiTool: _preload_app_names_cache from loaded_apps failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"EpiTool: _preload_app_names_cache from applist failed: {exc}")
...
def _load_applist_into_memory() -> None:
    ...
    with APPLIST_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("EpiTool: Applist file not found, skipping load")
            APPLIST_LOADED = True  # Mark as loaded to avoid repeated checks
            return
        
        try:
            logger.log("EpiTool: Loading applist into memory...")
                ...
                for entry in data:
                    if isinstance(entry, dict):
                        ...
                        if appid and name and isinstance(name, str) and name.strip():
                            ...
                            count += 1
                logger.log(f"EpiTool: Loaded {count} app names from applist into memory")
            else:
                logger.warn("EpiTool: Applist file has invalid format (expected array)")
            
            APPLIST_LOADED = True
        except Exception as exc:
            logger.warn(f"EpiTool: Failed to load applist into memory: {exc}")
...
def _ensure_applist_file() -> None:
    ...
    if os.path.exists(file_path):
        logger.log("EpiTool: Applist file already exists, skipping download")
        return
    
    logger.log("EpiTool: Applist file not found, downloading...")
    client = ensure_http_client("EpiTool: DownloadApplist")
    ...
    try:
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warn("EpiTool: Downloaded applist has invalid format (expected array)")
                return
        except json.JSONDecodeError as exc:
            logger.warn(f"EpiTool: Downloaded applist is not valid JSON: {exc}")
            return
        ...
        logger.log(f"EpiTool: Successfully downloaded and saved applist file ({len(data)} entries)")
    except Exception as exc:
        logger.warn(f"EpiTool: Failed to download applist file: {exc}")
...
def init_applist() -> None:
    ...
    try:
        ...
        logger.warn(f"EpiTool: Applist initialization failed: {exc}")
...
def _process_and_install_lua(appid: int, zip_path: str) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for name in names:
                try:
                    ...
                    if name.lower().endswith(".manifest"):
                        ...
                        logger.log(f"EpiTool: Extracted manifest -> {out_path}")
                except Exception as manifest_exc:
                    logger.warn(f"EpiTool: Failed to extract manifest {name}: {manifest_exc}")
        except Exception as depot_exc:
            logger.warn(f"EpiTool: depotcache extraction failed: {depot_exc}")
        ...
        logger.log(f"EpiTool: Installed lua -> {dest_file}")
...
def _download_zip_for_app(appid: int):
    client = ensure_http_client("EpiTool: download")
    apis = load_api_manifest()
    if not apis:
        logger.warn("EpiTool: No enabled APIs in manifest")
    ...
    for api in apis:
        ...
        logger.log(f"EpiTool: Trying API '{name}' -> {url}")
        try:
            headers = {"User-Agent": USER_AGENT}
            if _is_download_cancelled(appid):
                logger.log(f"EpiTool: Download cancelled before contacting API '{name}'")
                return
            with client.stream("GET", url, headers=headers, follow_redirects=True) as resp:
                code = resp.status_code
                logger.log(f"EpiTool: API '{name}' status={code}")
                ...
                with open(dest_path, "wb") as output:
                    for chunk in resp.iter_bytes():
                        if not chunk:
                            continue
                        if _is_download_cancelled(appid):
                            logger.log(f"EpiTool: Download cancelled mid-stream for appid={appid}")
                            raise RuntimeError("cancelled")
                        output.write(chunk)
                        ...
                        if _is_download_cancelled(appid):
                            logger.log(f"EpiTool: Download cancelled after writing chunk for appid={appid}")
                            raise RuntimeError("cancelled")
                logger.log(f"EpiTool: Download complete -> {dest_path}")

                if _is_download_cancelled(appid):
                    logger.log(f"EpiTool: Download marked cancelled after completion for appid={appid}")
                    raise RuntimeError("cancelled")
                ...
                try:
                    with open(dest_path, "rb") as fh:
                        ...
                        if magic not in (b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08"):
                            ...
                            logger.warn(
                                f"EpiTool: API '{name}' returned non-zip file (magic={magic.hex()}, size={file_size}, preview={content_preview[:50]})"
                            )
                            try:
                                os.remove(dest_path)
                            except Exception:
                    ...
                    logger.warn("EpiTool: Downloaded file not found after download")
                    continue
                except Exception as validation_exc:
                    logger.warn(f"EpiTool: File validation failed for API '{name}': {validation_exc}")
                    try:
                        os.remove(dest_path)
                    except Exception:
                        pass
                ...
                try:
                    if _is_download_cancelled(appid):
                        logger.log(f"EpiTool: Processing aborted due to cancellation for appid={appid}")
                        raise RuntimeError("cancelled")
                    _set_download_state(appid, {"status": "processing"})
                    _process_and_install_lua(appid, dest_path)
                    if _is_download_cancelled(appid):
                        logger.log(f"EpiTool: Installation complete but marked cancelled for appid={appid}")
                        raise RuntimeError("cancelled")
                    try:
                    ...
                    if isinstance(install_exc, RuntimeError) and str(install_exc) == "cancelled":
                        try:
                            ...
                            pass
                        logger.log(f"EpiTool: Cancelled download cleanup complete for appid={appid}")
                        return
                    logger.warn(f"EpiTool: Processing failed -> {install_exc}")
                    _set_download_state(
            ...
            if str(cancel_exc) == "cancelled":
                try:
                    ...
                    pass
                logger.log(f"EpiTool: Download cancelled and cleaned up for appid={appid}")
                return
            logger.warn(f"EpiTool: Runtime error during download for appid={appid}: {cancel_exc}")
            _set_download_state(appid, {"status": "failed", "error": str(cancel_exc)})
            return
        except Exception as err:
            logger.warn(f"EpiTool: API '{name}' failed with error: {err}")
            continue

    _set_download_state(appid, {"status": "failed", "error": "Not available on any API"})


def start_add_via_epitool(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})

    logger.log(f"EpiTool: StartAddViaEpiTool appid={appid}")
...
def delete_epitool_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    for path in paths:
        try:
            ...
            logger.warn(f"EpiTool: Failed to delete {path}: {exc}")
    try:
...
def get_icon_data_url() -> str:
    try:
        ...
        logger.warn(f"EpiTool: GetIconDataUrl failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def has_epitool_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
...
def cancel_add_via_epitool(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    logger.log(f"EpiTool: Cancellation requested for appid={appid}")
    return json.dumps({"success": True})
...
def get_installed_lua_scripts() -> str:
    ...
    try:
            ...
            for filename in os.listdir(target_dir):
                ...
                if filename.endswith(".lua") or filename.endswith(".lua.disabled"):
                    try:
                        ...
                        logger.warn(f"EpiTool: Failed to process Lua file {filename}: {exc}")
                        continue

        except Exception as exc:
            logger.warn(f"EpiTool: Failed to scan stplug-in directory: {exc}")
        ...
        logger.warn(f"EpiTool: Failed to get installed Lua scripts: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


__all__ = [
    "cancel_add_via_epitool",
    "delete_epitool_for_app",
    "dismiss_loaded_apps",
    "fetch_app_name",
    "get_add_status",
    "get_icon_data_url",
