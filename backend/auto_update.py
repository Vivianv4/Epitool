"""Auto-update utilities for the EpiTool backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"EpiTool updated to {new_version}. Please restart Steam."
        return "EpiTool update applied. Please restart Steam."
...
def _fetch_github_latest(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    asset_name = str(cfg.get("asset_name", "epitoolplugin.zip")).strip()
...
def check_for_update_once() -> str:
    ...
    if isinstance(gh_cfg, dict):
        owner = str(gh_cfg.get("owner", "")).strip()
        repo = str(gh_cfg.get("repo", "")).strip()
        if not owner or not repo:
            # No GitHub config, skip update check
            return ""
    ...
    try:
        ...
        return f"EpiTool updated to {latest_version}. Please restart Steam."
...
def _check_and_donate_keys() -> None:
    """Donate keys functionality disabled."""
    # Donation functionality has been removed
    pass
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"EpiTool: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("EpiTool: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"EpiTool: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"EpiTool: CheckForUpdatesNow failed: {exc}")
