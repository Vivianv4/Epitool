# Build Script for EpiTool Steam Plugin
# Creates a ZIP file ready for distribution

param(
    [string]$OutputName = "epitoolplugin.zip",
    [switch]$Clean
)
...
Write-Host "=== EpiTool Build Script ===" -ForegroundColor Cyan
...
try {
    ...
    if (Test-Path "scripts\validate_locales.py") {
        ...
        Write-Host "Locale validation script not found, skipping..." -ForegroundColor Yellow
    }
} catch {
...
$TempZip = Join-Path $env:TEMP "epitool_build_$(Get-Date -Format 'yyyyMMddHHmmss').zip"
